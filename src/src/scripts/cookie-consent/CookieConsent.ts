...
export default class CookieConsent {
    ...
    protected cookieConsentName = "CookieConsent";
    protected alreadyConfigured = false;
    protected showDetailsOnly = false;
    protected onClickClassName = ".on-click";
    protected showBannerBoolean = true;
    protected functions = {
    ...
    constructor(props=null) {
        if (props !== null) {
            if (props.locale !== void 0) this.setLocale(props.locale);
            this.prepareDictionnary();
            if (props.content !== void 0) this.setContent(props.content);
            if (props.cookies !== void 0) this.setCookieTypes(props.cookies);
            if (props.type !== void 0) this.data.type = props.type;
            if (props.labels !== void 0) this.data.labels = props.labels;
            if (props.documentNode !== void 0) this.data.documentNode = props.documentNode;
        }
    }

    protected prepareDictionnary() {
    ...
    public setCookieConsentName(name) {
        this.cookieConsentName = name;
        return this;
    }

    public displayInlineContent() {
        let cookies = "";
    ...
    public setCookieTypes(cookieTypes) {
        this.data.cookies = cookieTypes;
        return this;
    }

    public addCookieType(cookieData) {
        this.data.cookies[cookieData.name] = cookieData;
        return this;
    }

    public addCallBack(cookieName, callback) {
    ...
    public defineCookie(value) {
        Cookies.set(this.cookieConsentName, value, { expires: 365 });
        return this;
    }

    public getCookie(useJSON=true) {
        if (useJSON === true )
    ...
    protected toggleDisplay(elm : HTMLElement) {
        elm.style.display = elm.style.display == "none" ? "inline-block" : "none";
        return this;
    }

    protected hideElement(elm : HTMLElement) {
        elm.style.display = "none";
        return this;
    } 

    protected defineEventListeners() { 
    ...
    protected toggleNext(elm : HTMLElement) {
    ...
    public createNode() {
        ...
        if (this.data.type === "popup") document.body.appendChild(div.firstChild);
        else if (this.data.type === "inline") document.querySelector(this.data.documentNode).appendChild(div.firstChild)
    }
    ...
    public createBlock(data, index, hideNextToggleButton = false ) {
        let rows = "";
    ...
    public showDetails () {
        ...
        Object.entries(this.DOMelements.wrapper).forEach(
            ([key, elm]) => elm.style.display = "inline-block"
        );
    ...
    public togglePopup() {
        ...
        // } else {
            Object.entries(this.DOMelements.wrapper).forEach(
                ([key, elm]) => this.toggleDisplay(elm)
            );
        // }
    ...
    public toggleDetails() {
        Object.entries(this.DOMelements.content).forEach(
            ([key, elm]) => this.toggleDisplay(elm)
        );
